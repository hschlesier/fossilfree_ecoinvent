...
def import_emission_factors():
    ...
    emission_data        = pd.read_excel(parent_dir / 'data/raw/emission_factors.xlsx', index_col=0)
...
def import_fossil_identifiers():
    ...
    global identifiers
    identifiers = []
    with open(parent_dir / "data/raw/fossil_identifier.txt", "r") as file:
        for line in file:
...
def import_input_map():
    parent_dir = Path.cwd().parent
    with open(parent_dir / 'data' / 'raw' /'input_mapping.json', 'r') as f:
...
def import_not_electrifiable_fuels():
    ...
    with open(parent_dir / 'data' / 'raw' /'not_electrifiable_fuels.txt', "r") as file:
        for line in file:
...
def import_fossil_refinery_processes():
    ...
    with open(parent_dir / 'data' / 'raw' / 'fossil_refinery_processes.txt', "r") as file:
        for line in file:
...
def import_not_electrifiable_processes():
    ...
    with open(parent_dir / 'data' / 'raw' / "not_electrifiable_processes.txt", "r") as file:
        for line in file:
...
def import_group_locations():
    ...
    global group_locs
    group_locs = []
    with open(parent_dir / 'data' / 'raw'/ "group_locations.txt", "r") as file:
        for line in file:
...
def import_electrifiable_processes():
    ...
    parent_dir = Path.cwd().parent
    with open(parent_dir / 'data' /'raw' /"electrifiable_processes.json", "r") as json_file:
...
def import_synfuel_dict():
    ...
    df           = pd.read_excel(parent_dir /'data'/'raw'/ 'synfuel_dict.xlsx', index_col=0)
...
def get_technosphere_map_dict(from_version='3.8', to_version='3.9'):
    parent_dir = Path.cwd().parent
    match_df_raw = pd.read_excel(parent_dir/ 'data/raw/technosphere_mapping.xlsx')
